...
setwd('C:/University of Florida/My research/CAMDA 2020')
aa<-read.csv('weather_meta.csv',header=T)
#### Summer ####
...
## Summer pressure ##
# summer_pressure<-grep("pressure",colnames(summerweather),value=TRUE)
# summer_pressureweather<-data_weather_season_2%>%select(all_of(summer_pressure))
# rownames(summer_pressureweather)<-data_weather_season_2$City
# NbClust(summer_pressureweather,method="kmeans")
# fviz_nbclust(summer_pressureweather,kmeans,method="silhouette")
# fviz_nbclust(summer_pressureweather,kmeans,method="wss")
# # gap_stat<-clusGap(summer_pressureweather,FUN=kmeans,nstart=25,K.max=10,B=100)
# # fviz_gap_stat(gap_stat)
# k2<-kmeans(summer_pressureweather,centers=3,nstart=25)
# fviz_cluster(k2, data = summer_pressureweather)
...
## winter pressure ##
# winter_pressure<-grep("pressure",colnames(winterweather),value=TRUE)
# winter_pressureweather<-data_weather_season_2%>%select(all_of(winter_pressure))
# rownames(winter_pressureweather)<-data_weather_season_2$City
# NbClust(winter_pressureweather,method="kmeans")
# fviz_nbclust(winter_pressureweather,kmeans,method="silhouette")
# fviz_nbclust(winter_pressureweather,kmeans,method="wss")
# # gap_stat<-clusGap(winter_pressureweather,FUN=kmeans,nstart=25,K.max=10,B=1000)
# # fviz_gap_stat(gap_stat)
# k2<-kmeans(winter_pressureweather,centers=2,nstart=25)
# fviz_cluster(k2, data = winter_pressureweather)
...
save.image('Clustering winter_summer_1.RData')
save(list=c('data_all'),file='weather_data_1.RData')
